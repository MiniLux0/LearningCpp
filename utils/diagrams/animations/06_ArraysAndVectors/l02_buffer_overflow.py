...
class L02BufferOverflow(BaseLearningScene):
    def construct(self):
        ...
        code_lines = VGroup(
            Text("int secreta{100};", font="Consolas", font_size=13, color=self.COLOR_GOLD),
            Text("int arr[3]{10, 20, 30}; // Indices 0..2", font="Consolas", font_size=13, color=self.COLOR_CYAN),
            Text("\n// Acceso Ilegal Fuera de Limite", font="Consolas", font_size=12, color=self.COLOR_MUTED),
            Text("arr[3] = 999; // Corrupcion!", font="Consolas", font_size=13, color=self.COLOR_RED, weight=BOLD)
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.14).move_to(code_bg.get_center() + DOWN * 0.05)

        # PANEL DERECHO: Tarjeta de Memoria Stack
        ram_group, ram_bg = self.create_card_panel(width=5.8, height=3.3, title="Stack Frame (Memoria)", subtitle="Variables en Direcciones Consecutivas")
        ...
        self.play(Write(code_lines[2]), Write(code_lines[3]), run_time=0.8)
